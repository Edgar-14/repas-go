...
import { TransactionType, NavigationProps } from '../types';
...
const PaymentsScreen: React.FC<NavigationProps> = ({ navigation }) => {
    ...
    const aggregates = useMemo(() => {
        ...
        (transactions || []).forEach((tx) => {
            ...
            if (type === TransactionType.CARD_ORDER_TRANSFER) {
                if (d >= startOfWeek) weeklyEarnings += amount;
                if (d >= startOfMonth) monthlyEarnings += amount;
                cardOrders += 1;
            } else if (type === TransactionType.CASH_ORDER_ADEUDO) {
                cashOrders += 1;
            } else if (type === TransactionType.TIP_CARD_TRANSFER) {
                totalTips += amount;
            } else if (type === TransactionType.BENEFITS_TRANSFER || type === TransactionType.DISTANCE_BONUS || type === TransactionType.TIME_BONUS) {
                totalBonuses += amount;
            }
        });
    ...
    const getTransactionIcon = (type: TransactionType): 'plus' | 'minus' | 'wallet' => {
        switch (type) {
            case TransactionType.CARD_ORDER_TRANSFER:
            case TransactionType.TIP_CARD_TRANSFER:
            case TransactionType.BENEFITS_TRANSFER:
            case TransactionType.DISTANCE_BONUS:
            case TransactionType.TIME_BONUS:
                return 'plus';
            case TransactionType.CASH_ORDER_ADEUDO:
                return 'minus';
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView style={styles.content}>
                ...
                {/* Deuda pendiente */}
                {pendingDebts > 0 && (
                    <View style={[styles.balanceCard, styles.debtCard]}>
                        <Text style={styles.debtLabel}>Deuda Pendiente</Text>
                        <Text style={styles.debtAmount}>${Number(pendingDebts).toFixed(2)}</Text>
                        <Text style={styles.debtNote}>Paga tu deuda para seguir recibiendo pedidos en efectivo</Text>
                    </View>
                )}
