...
import type { RootStackParamList, WalletTransaction } from '../types/index';
import { TransactionType } from '../types/index';
import { StackNavigationProp } from '@react-navigation/stack';
import { useDispatch, useSelector } from 'react-redux';
import type { RootState, AppDispatch } from '../store/index';
import { fetchTransactionHistory, listenToWalletBalance, requestWithdrawal } from '../store/slices/walletSlice';

const ArrowLeft = (props: { color: string; size?: number }) => (
  <MaterialCommunityIcons name="arrow-left" size={props.size || 24} color={props.color} />
);
const WalletIcon = (props: { color: string; size?: number }) => (
  <MaterialCommunityIcons name="wallet" size={props.size || 24} color={props.color} />
);
const Wifi = (props: { color: string; size?: number }) => (
  <MaterialCommunityIcons name="wifi" size={props.size || 24} color={props.color} />
);
const Lock = (props: { color: string; size?: number }) => (
  <MaterialCommunityIcons name="lock" size={props.size || 24} color={props.color} />
);
const AlertCircle = (props: { color: string; size?: number }) => (
  <MaterialCommunityIcons name="alert-circle-outline" size={props.size || 24} color={props.color} />
);
const Clock = (props: { color: string; size?: number }) => (
  <MaterialCommunityIcons name="clock-outline" size={props.size || 24} color={props.color} />
);
const ShoppingBag = (props: { color: string; size?: number }) => (
  <MaterialCommunityIcons name="shopping-outline" size={props.size || 24} color={props.color} />
);
const Gift = (props: { color: string; size?: number }) => (
  <MaterialCommunityIcons name="gift-outline" size={props.size || 24} color={props.color} />
);
const ArrowUpCircle = (props: { color: string; size?: number }) => (
  <MaterialCommunityIcons name="arrow-up-circle-outline" size={props.size || 24} color={props.color} />
);
const ArrowDownCircle = (props: { color: string; size?: number }) => (
  <MaterialCommunityIcons name="arrow-down-circle-outline" size={props.size || 24} color={props.color} />
);
const DollarSign = (props: { color: string; size?: number }) => (
  <MaterialCommunityIcons name="currency-usd" size={props.size || 24} color={props.color} />
);
...
const WalletScreen: React.FC = () => {
  ...
  const transactionIcons = {
    ...
    [TransactionType.MARKET_COMMISSION]: { icon: ShoppingBag, color: '#8B5CF6' },
    [TransactionType.DISTANCE_BONUS]: { icon: Gift, color: '#10B981' },
    [TransactionType.TIME_BONUS]: { icon: Gift, color: '#F59E0B' },
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.card}>
          ...
          {transactions.slice(0, 5).map((tx: WalletTransaction, index: number) => {
            const { icon: Icon, color } = getTransactionIcon(tx.type);
            const isPositive = tx.amount > 0;

            const date = tx.timestamp
              ? (tx.timestamp as any).toDate
                ? (tx.timestamp as any).toDate().toLocaleDateString()
                : new Date(tx.timestamp as any).toLocaleDateString()
              : tx.date || 'N/A';
      ...
      <View style={styles.footer}>
        <TouchableOpacity 
          ...
          onPress={() => {
            Alert.alert(
              'Pagar Deuda',
              `Deuda actual: $${Number(pendingDebts || 0).toFixed(2)}\n\nPara pagar tu deuda, contacta a soporte o realiza un depósito.`,
              [{ text: 'Entendido' }]
            );
          }}
        >
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            ...
            Alert.alert(
              'Solicitar Retiro',
              `Saldo disponible: $${Number(balance).toFixed(2)}\n\n¿Cuánto deseas retirar?`,
              [
                { text: 'Cancelar', style: 'cancel' },
                {
                  ...
                  onPress: () => {
                    if (driver?.uid) {
                      dispatch(requestWithdrawal({ driverId: driver.uid, amount: balance }));
                    }
                  }
                }
              ]
