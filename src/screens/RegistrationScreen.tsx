...
const RegistrationScreen: React.FC = () => {
    ...
    const handleFilePick = async (name: string) => {
        try {
            // En React Native, usarías react-native-document-picker
            // Para esta versión, simulamos la selección pero con estructura real
            // En producción, integrar con DocumentPicker.pick()
            const mockFile = { 
                name: `${name}-seleccionado.pdf`, 
                size: 12345, 
                type: 'application/pdf',
                uri: `file:///path/to/${name}.pdf` // URI real para upload
            };
