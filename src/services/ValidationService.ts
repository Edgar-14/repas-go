// Servicio de Validación para BeFast GO - Integración con Ecosistema
import { firestore, functions, COLLECTIONS, CLOUD_FUNCTIONS } from '../config/firebase';
import { ValidationResult, CriticalValidation } from '../types';

/**
 * Servicio de validación 360° integrado con el ecosistema BeFast
 * Implementa todas las validaciones críticas antes de asignar pedidos
 */
class ValidationService {
  /**
   * Validación completa del conductor antes de aceptar un pedido
   * Esta función se llama antes de cada asignación de pedido
   */
  async validateDriverForOrderAssignment(driverId: string): Promise<ValidationResult> {
    try {
      ...
      console.error('Error en validación de conductor:', error);
      return {
        canReceiveOrders: false,
  ...
  async validateIMSS(driverId: string): Promise<boolean> {
    try {
      ...
      console.error('Error validando IMSS:', error);
      return false;
    }
  }
  ...
  async callValidateOrderAssignment(
    ...
    try {
      ...
      console.error('Error calling validateOrderAssignment:', error);
      return {
        approved: false,
        canReceiveOrders: false,
  ...
  async canDriverOperate(driverId: string): Promise<{
    ...
    try {
      ...
      console.error('Error checking if driver can operate:', error);
      return {
        canOperate: false,
