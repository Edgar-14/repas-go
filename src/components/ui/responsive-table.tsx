 ...
 * Handles table display across different screen sizes with mobile card layout
 */
    ...
    <Card className={cn("w-full", className)}>
      <CardHeader>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          {title && <CardTitle className="text-xl min-w-0">{title}</CardTitle>}
...
// Legacy components for backward compatibility - consider these deprecated
  ...
  return (
    <tr 
      className={cn(
        ...
        isMobile && 'block border-b-2 mb-4 pb-4',
...
export function ResponsiveTableCell({ 
  ...
  return (
    <td className={cn(
      'px-4 py-3',
      isMobile && 'block text-right border-none',
      isMobile && label && 'before:content-[attr(data-label)] before:float-left before:font-medium before:text-gray-600',
      className
    )}
    data-label={isMobile && label ? `${label}: ` : undefined}
    >
      {isMobile && label && !className?.includes('before:') && (
        <span className="font-medium text-gray-600 float-left">
          {label}:{' '}
        </span>
      )}
...
export function TableCardField({ label, value, className }: TableCardFieldProps) {
  return (
...
/**
 * Hook to determine table display mode
 */
