...
const TrackingMap: React.FC<TrackingMapProps> = ({
  ...
  // Usar AnimatedRegion para la ubicación del conductor
  const driverLocation = useRef(
    new AnimatedRegion({
      latitude: pickupLocation?.latitude || DEFAULT_LOCATION.latitude,
      longitude: pickupLocation?.longitude || DEFAULT_LOCATION.longitude,
      latitudeDelta: 0,
      longitudeDelta: 0,
    })
  ).current;
  ...
  const currentOrigin: Location | null = pickupLocation || null;
  const currentDestination: Location | null = deliveryLocation || null;

  // --- EFECTOS ---

  // 1. Obtener ubicación del conductor en tiempo real y animar el marcador
  useEffect(() => {
    if (!driverId) {
      setLoading(false);
      return;
        ...
        (doc) => {
          if (doc.exists()) {
            ...
            if (location && location.latitude && location.longitude) {
              ...
              driverLocation.timing({ ...newCoordinate, useNativeDriver: false, duration: 1000 }).start();
            }
          }
          setLoading(false);
        },
